...
@pytest.mark.parametrize("array_dtype", ["T", object])
@pytest.mark.parametrize("shape", [(), (2,), (2, 1)])
def test_pystr_scalar_ufunc_outer_preserves_nulls(array_dtype, shape):
    arr = np.full(shape, "x", dtype=array_dtype)
    for result, expected in [(np.add.outer(arr, "y\0"), "xy\0"),
                             (np.add.outer("y\0", arr), "y\0x")]:
        if shape == ():
            # These dtypes return Python str scalars for zero-dimensional
            # results; array conversion would discard the trailing null.
            assert type(result) is str
            assert result == expected
        else:
            assert_array_equal(result, np.full(shape, expected, dtype=array_dtype),
                               strict=True)


@pytest.mark.parametrize("array_dtype", ["T", object])
def test_pystr_scalar_ufunc_at_preserves_nulls(array_dtype):
    arr = np.array(["x", "z"], dtype=array_dtype)
    np.add.at(arr, [0, 0], "y\0")
    assert arr.tolist() == ["xy\0y\0", "z"]
